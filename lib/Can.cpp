...
// ajout de methode a la classe fournie par le professeur
...
uint8_t Can::lecture8bits(uint8_t pos){
   ...
   uint16_t mesure16bits = lecture(pos);
            mesure16bits = mesure16bits >> DECALAGE;
   uint8_t  mesure8bits = static_cast<uint8_t>(mesure16bits);
